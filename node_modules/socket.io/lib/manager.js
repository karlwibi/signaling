...
Manager.prototype.handleClient = function (data, req) {
  ...
  if (undefined != data.query.disconnect) {
    if (this.transports[data.userid] && this.transports[data.userid].open) {
      this.transports[data.userid].onForcedDisconnect();
    } else {
      this.store.publish('disconnect-force:' + data.userid);
    }
  ...
  var transport = new transports[data.transport](this, data, req)
    , handshaken = this.handshaken[data.userid];
  ...
  if (handshaken) {
    if (transport.open) {
      if (this.closed[data.userid] && this.closed[data.userid].length) {
        transport.payload(this.closed[data.userid]);
        this.closed[data.userid] = [];
      }

      this.onOpen(data.userid);
      this.store.publish('open', data.userid);
      this.transports[data.userid] = transport;
    }

    if (!this.connected[data.userid]) {
      this.onConnect(data.userid);
      this.store.publish('connect', data.userid);

      // flag as used
      delete handshaken.issued;
      this.onHandshake(data.userid, handshaken);
      this.store.publish('handshake', data.userid, handshaken);

      // initialize the socket for all namespaces
      for (var i in this.namespaces) {
        if (this.namespaces.hasOwnProperty(i)) {
          var socket = this.namespaces[i].socket(data.userid, true);

          // echo back connect packet and fire connection event
          if (i === '') {
            this.namespaces[i].handlePacket(data.userid, { type: 'connect' });
          }
        }
      }

      this.store.subscribe('message:' + data.userid, function (packet) {
        self.onClientMessage(data.userid, packet);
      });

      this.store.subscribe('disconnect:' + data.userid, function (reason) {
        self.onClientDisconnect(data.userid, reason);
      });
    }
  } else {
    if (transport.open) {
