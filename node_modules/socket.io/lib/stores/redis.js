 ...
 * Redis store.
 * Options:
 *     - nodeId (fn) gets an userid that uniquely identifies this node
...
function Redis (opts) {
  ...
  // node userid to uniquely identify this node
  var nodeId = opts.nodeId || function () {
    // by default, we generate a random userid
...
Client.prototype.get = function (key, fn) {
  this.store.cmd.hget(this.userid, key, fn);
  return this;
};
...
Client.prototype.set = function (key, value, fn) {
  this.store.cmd.hset(this.userid, key, value, fn);
  return this;
};
...
Client.prototype.del = function (key, fn) {
  this.store.cmd.hdel(this.userid, key, fn);
  return this;
};
...
Client.prototype.has = function (key, fn) {
  this.store.cmd.hexists(this.userid, key, function (err, has) {
...
Client.prototype.destroy = function (expiration) {
  if ('number' != typeof expiration) {
    this.store.cmd.del(this.userid);
  } else {
    this.store.cmd.expire(this.userid, expiration);
  }
