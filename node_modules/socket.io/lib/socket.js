...
function Socket (manager, id, nsp, readable) {
  ...
  this.store = this.manager.store.client(this.userid);
...
Socket.prototype.__defineGetter__('handshake', function () {
  return this.manager.handshaken[this.userid];
});
...
Socket.prototype.__defineGetter__('transport', function () {
  return this.manager.transports[this.userid].name;
});
...
Socket.prototype.join = function (name, fn) {
  ...
  this.manager.onJoin(this.userid, name);
  this.manager.store.publish('join', this.userid, name);
...
Socket.prototype.leave = function (name, fn) {
  ...
  this.manager.onLeave(this.userid, name);
  this.manager.store.publish('leave', this.userid, name);
...
Socket.prototype.packet = function (packet) {
  if (this.flags.broadcast) {
    ...
    this.namespace.in(this.flags.room).except(this.userid).packet(packet);
  } else {
...
Socket.prototype.dispatch = function (packet, volatile) {
  if (this.manager.transports[this.userid] && this.manager.transports[this.userid].open) {
    this.manager.transports[this.userid].onDispatch(packet, volatile);
  } else {
    if (!volatile) {
      this.manager.onClientDispatch(this.userid, packet, volatile);
    }

    this.manager.store.publish('dispatch:' + this.userid, packet, volatile);
  }
};
...
Socket.prototype.disconnect = function () {
  if (!this.disconnected) {
    ...
    if ('' === this.namespace.name) {
      if (this.manager.transports[this.userid] && this.manager.transports[this.userid].open) {
        this.manager.transports[this.userid].onForcedDisconnect();
      } else {
        this.manager.onClientDisconnect(this.userid);
        this.manager.store.publish('disconnect:' + this.userid);
      }
    } else {
      this.packet({type: 'disconnect'});
      this.manager.onLeave(this.userid, this.namespace.name);
