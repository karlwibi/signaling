...
Transport.prototype.setHandlers = function () {
  ...
  this.store.subscribe('heartbeat-clear:' + this.userid, function () {
    self.onHeartbeatClear();
  });

  this.store.subscribe('disconnect-force:' + this.userid, function () {
    self.onForcedDisconnect();
  });

  this.store.subscribe('dispatch:' + this.userid, function (packet, volatile) {
...
Transport.prototype.clearHandlers = function () {
  if (this.handlersSet) {
    this.store.unsubscribe('disconnect-force:' + this.userid);
    this.store.unsubscribe('heartbeat-clear:' + this.userid);
    this.store.unsubscribe('dispatch:' + this.userid);
...
Transport.prototype.setCloseTimeout = function () {
  if (!this.closeTimeout) {
    ...
    this.closeTimeout = setTimeout(function () {
      self.log.debug('fired close timeout for client', self.userid);
      self.closeTimeout = null;
      self.end('close timeout');
    }, this.manager.get('close timeout') * 1000);

    this.log.debug('set close timeout for client', this.userid);
  }
};
...
Transport.prototype.clearCloseTimeout = function () {
  if (this.closeTimeout) {
    ...
    this.log.debug('cleared close timeout for client', this.userid);
  }
};
...
Transport.prototype.setHeartbeatTimeout = function () {
  if (!this.heartbeatTimeout && this.manager.enabled('heartbeats')) {
    ...
    this.heartbeatTimeout = setTimeout(function () {
      self.log.debug('fired heartbeat timeout for client', self.userid);
      self.heartbeatTimeout = null;
      self.end('heartbeat timeout');
    }, this.manager.get('heartbeat timeout') * 1000);

    this.log.debug('set heartbeat timeout for client', this.userid);
  }
};
...
Transport.prototype.clearHeartbeatTimeout = function () {
  if (this.heartbeatTimeout && this.manager.enabled('heartbeats')) {
    ...
    this.log.debug('cleared heartbeat timeout for client', this.userid);
  }
};
...
Transport.prototype.setHeartbeatInterval = function () {
  if (!this.heartbeatInterval && this.manager.enabled('heartbeats')) {
    ...
    this.log.debug('set heartbeat interval for client', this.userid);
  }
};
...
Transport.prototype.heartbeat = function () {
  if (this.open) {
    this.log.debug('emitting heartbeat for client', this.userid);
...
Transport.prototype.onMessage = function (packet) {
  var current = this.manager.transports[this.userid];
  ...
  if ('heartbeat' == packet.type) {
    ...
    if (current && current.open) {
      ...
      this.store.publish('heartbeat-clear:' + this.userid);
    }
  } else {
    if ('disconnect' == packet.type && packet.endpoint == '') {
      ...
      if (current) {
        ...
        this.store.publish('disconnect-force:' + this.userid);
      }

      return;
    }

    if (packet.userid && packet.ack != 'data') {
      ...
      if (current && current.open) {
        ...
        this.manager.onClientDispatch(this.userid, ack);
        this.store.publish('dispatch:' + this.userid, ack);
      }
    }

    // handle packet locally or publish it
    if (current) {
      this.manager.onClientMessage(this.userid, packet);
    } else {
      this.store.publish('message:' + this.userid, packet);
    }
  }
};
...
Transport.prototype.clearHeartbeatInterval = function () {
  if (this.heartbeatInterval && this.manager.enabled('heartbeats')) {
    ...
    this.log.debug('cleared heartbeat interval for client', this.userid);
  }
};
...
Transport.prototype.onClose = function () {
  if (this.open) {
    ...
    this.clearHandlers();
    this.open = false;
    this.manager.onClose(this.userid);
    this.store.publish('close', this.userid);
  }
};
...
Transport.prototype.end = function (reason) {
  if (!this.disconnected) {
    ...
    var local = this.manager.transports[this.userid];
    ...
    if (local) {
      this.manager.onClientDisconnect(this.userid, reason, true);
    } else {
      this.store.publish('disconnect:' + this.userid, reason);
    }
  }
};
